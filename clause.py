...
# -------------------
# Extract Key Details for Colored Boxes
# -------------------
def extract_key_details(document: str) -> dict:
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "user", "content": f"""
                From the following contract, extract:
                1. Contract Party (counterparty name, not your own company)
                2. Start Date
                3. End Date
                4. Contract Value (currency + amount if possible)
                5. Payment Terms (summary)
                Return as JSON with keys: contract_party, start_date, end_date, value, payment_terms.
...
def display_colored_boxes(details: dict):
    colors = {
        "contract_party": "#FFDDC1",  # Light Peach
        "start_date": "#C1FFD7",      # Light Green
        "end_date": "#C1E1FF",        # Light Blue
        "value": "#FFF5C1",           # Light Yellow
        "payment_terms": "#E1C1FF"    # Light Purple
    }
    for key, value in details.items():
        label = key.replace("_", " ").title()
        st.markdown(
            f"""
            <div style="background-color:{colors[key]}; padding:15px; border-radius:10px; margin-bottom:10px;">
                <b>{label}:</b> {value}
            </div>
            """,
...
# -------------------
def main():
    st.title("📜 Contract Analyzer with Clause Checker & Key Details")
    ...
    if uploaded_file is not None:
        ...
        if st.button("🔍 Analyze Contract"):
            ...
            display_colored_boxes(key_details)
